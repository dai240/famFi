...
export async function GET(request: Request) {
  try {
    ...
    const result = await withUserDb(user.id, async tx => {
      const base = Prisma.sql`from famfi.expenses e where e.user_id=${user.id}::uuid and e.reimbursement_status='required'`;
      const balance = Prisma.sql`e.reimbursement_amount - coalesce((select sum(s.amount) from famfi.settlements s where s.user_id=e.user_id and s.expense_id=e.id and s.cancelled_at is null),0)`;
...
export async function POST(request: Request) {
  try {
    ...
    const result = await withLedgerDb(user.id, async tx => {
      const expense = await lockedExpense(tx, user.id, input.expenseId);
      ...
      await tx.$executeRaw`insert into famfi.settlements(id,user_id,expense_id,amount,date,from_party_id,to_party_id,memo) values (${input.id}::uuid,${user.id}::uuid,${input.expenseId}::uuid,${input.amount},${new Date(input.date+'T00:00:00Z')}::date,${expense.reimbursementFromPartyId}::uuid,${expense.reimbursementToPartyId}::uuid,${input.memo})`;
