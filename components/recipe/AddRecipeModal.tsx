...
export function AddRecipeModal({ isOpen, onClose, onSave, people }: AddRecipeModalProps) {
  ...
  const [steps, setSteps] = useState<RecipeStep[]>([
    { id: '1', stepNumber: 1, instruction: '', imageUrl: '', duration: undefined }
  ]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.title || !formData.category || !formData.createdBy) {
      return;
    }
    ...
    setSteps([{ id: '1', stepNumber: 1, instruction: '', imageUrl: '', duration: undefined }]);
    setNewTag('');
  };
