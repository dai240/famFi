...
const querySchema=z.object({entityType:z.enum(['expenses','category_entries','parties','payment_sources','settlements','recurring_rules','recurring_occurrences','planned_expenses','expense_summaries','household_notes']).optional(),entityId:z.string().min(1).max(60).optional(),page:z.coerce.number().int().min(1).max(100000).default(1)}).strict();
