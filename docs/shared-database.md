...
2026-09-09 に支出MVPと招待コードによる初回登録を実装しました。本人のメール確認・membership登録は完了しています。認証後の本番処理で見つかったPrismaコンパイラの同梱漏れを修正し、通常ログインの再確認待ちです。初期状態（支出0件）の暗号化バックアップとローカル復元は確認済み。最新状態は管理リポジトリの `docs/status.md` を確認してください。
