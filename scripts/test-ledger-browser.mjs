...
for(const [engineName,engine] of [['chromium',chromium],['webkit',webkit]]){
  ...
  try{
    ...
    for(const [width,height] of [[320,700],[390,844],[430,932],[390,600],[844,390],[1280,900]]){
      ...
      await choose(page,editor,'カテゴリ',parent+' / '+child);
      check(await editor.locator('.category-select .category-swatch').evaluate(el=>getComputedStyle(el).backgroundColor)==='rgb(54, 125, 145)','Edited color is reflected');
      await editor.getByRole('button',{name:'人物・支払元・立替',exact:true}).click();await choose(page,editor,'使った人',person);await choose(page,editor,'誰のため',fund);await choose(page,editor,'支払元',source);check((await editor.getByRole('combobox',{name:'支払者・資金の負担元',exact:true}).innerText()).includes(person),'Funding default is visibly selected');await choose(page,editor,'精算の要否','立替・精算が必要');await choose(page,editor,'返す側',fund);await choose(page,editor,'受け取る側（立替者）',person);await editor.getByRole('button',{name:'全額',exact:true}).click();check(await editor.getByLabel('精算対象額（円）').inputValue()==='3000','Full amount remains explicit');
