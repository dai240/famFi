...
export async function PUT(request:Request,context:{params:Promise<{id:string}>}) {
  try {
    ...
    const {version,confirmProvisional,...input}=updateRecurringSchema.parse(await readJson(request));
    return json(await withLedgerDb(user.id,async(tx,scope)=>{
      ...
      if(isProvisionalRule(old)&&!isProvisionalRule(input)&&!confirmProvisional)throw new ApiError(422,'実際の金額・支払元・確認日を確認してください。');
