...
export async function PUT(request: Request, context: Context) {
  try {
    ...
    const row = await withLedgerDb(user.id, async (tx, scope) => {
      ...
      const result = await tx.expense.updateMany({ where: { id, userId: scope.ledgerId, version },
...
export async function DELETE(request: Request, context: Context) {
  try {
    ...
    await withLedgerDb(user.id, async (tx, scope) => {
      const row = await lockedExpense(tx, scope.ledgerId, id);
      if (row.settlements.length) throw new ApiError(409, '精算履歴のある支出は削除できません。履歴を残して内容を訂正してください。');
      const deleted = await tx.expense.deleteMany({ where: { id, userId: scope.ledgerId, version } });
