...
async function login(page,token){await page.goto(base+'/login');await page.getByLabel('メールアドレス').fill('fixture0@example.invalid');await page.getByRole('button',{name:'確認コードを送信',exact:true}).click();await page.getByLabel('確認コード',{exact:true}).fill(token);await page.getByRole('button',{name:'ログイン',exact:true}).click();await page.waitForURL('**/expenses');await page.getByRole('button',{name:'マスタ管理',exact:true}).waitFor();}
for(const [engineName,engine] of [['chromium',chromium],['webkit',webkit]]){
  ...
  try{
    ...
    for(const [width,height] of [[320,700],[390,844],[390,600],[844,390],[1280,900]]){
      ...
      await page.getByRole('tab',{name:'定期支出',exact:true}).click();const workspace=page.locator('.recurring-workspace');await workspace.getByRole('tab',{name:'この月',exact:true}).click();await workspace.getByLabel('定期支出の表示月').fill('2031-02');await workspace.getByRole('button',{name:'追加',exact:true}).click();
      const ruleEditor=page.getByRole('dialog',{name:'定期支出を追加',exact:true});await ruleEditor.getByLabel('名称',{exact:true}).fill('電気 '+label);await ruleEditor.getByRole('radio',{name:'毎回入力（変動額）',exact:true}).check();
