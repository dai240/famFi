...
for(const [engineName,engine] of [['chromium',chromium],['webkit',webkit]]){
  ...
  try{
    ...
    for(const [index,[width,height]] of [[320,568],[390,844],[844,390],[1280,800]].entries()){
      ...
      await planItem.getByRole('button',{name:'支出を確定',exact:true}).click();editor=page.getByRole('dialog',{name:'誕生日 '+name+'を確定',exact:true});await expandExpenseFields(editor);check(await editor.getByLabel('精算対象額（円）').inputValue()==='5000','Planned amount initializes personal advance');await editor.getByLabel('金額（円）').fill('4800');await choose(page,editor,'費用の区分','特別費');await fit(page,editor);await editor.getByRole('button',{name:'保存',exact:true}).click();await editor.waitFor({state:'hidden'});await planItem.waitFor({state:'hidden'});await workspace.getByLabel('確定・中止済みも表示').check();await planItem.getByRole('button',{name:'登録済みの支出',exact:true}).waitFor();
