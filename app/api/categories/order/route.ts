...
export async function PUT(request: Request) {
  try {
    ...
    await withLedgerDb(user.id, async (tx, scope) => {
      const first = await tx.category.findUnique({ where: { userId_id: { userId: scope.ledgerId, id: input.entries[0].id } } });
      ...
      for (const [index, entry] of input.entries.entries()) await tx.category.update({ where: { userId_id: { userId: scope.ledgerId, id: entry.id } }, data: { sortOrder: (index+1)*10, version: { increment: 1 } } });
    });
