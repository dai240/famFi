...
import { ChevronUp, Pencil, Settings2 } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { ExpenseFields, formatYen } from '@/lib/expenses';
import { Masters, treatmentLabels } from '@/lib/ledger';
import { beneficiaryLabel, canSummarizePayment, paymentSourceGroups, paymentSuggestion, paymentSummary, personLabel } from '@/lib/household';
import { ReferenceSelect } from './ReferenceSelect';

export function PaymentFields({fields,masters,onChange,disabled=false,financialLocked=false,onManage,recurring=false,compact=false}:{
  fields:ExpenseFields;masters:Masters;onChange:(patch:Partial<ExpenseFields>)=>void;disabled?:boolean;financialLocked?:boolean;
  onManage?:(kind:'parties'|'payment-sources')=>void;recurring?:boolean;compact?:boolean;
}) {
  const [buyerOther,setBuyerOther]=useState(Boolean(fields.usedByText));
  const [expanded,setExpanded]=useState(!compact);
  const contentId=useId(),toggle=useRef<HTMLButtonElement>(null);
  const canCollapse=canSummarizePayment(fields,masters);
  const showFields=expanded||!canCollapse;
  ...
  const outcome=source&&<div className={`payment-outcome ${fields.paymentTreatment}`} role="status"><span>資金：{personLabel(source.fundingPartyId,masters)}</span><strong>{paymentSummary(fields,masters)}{fields.reimbursementStatus==='required'&&!recurring&&` ${fields.reimbursementAmount?formatYen(fields.reimbursementAmount):'金額未入力'}`}</strong></div>;
  return <section className="expense-details payment-fields" aria-label="支払情報">
    {compact&&<div className="field-heading compact-heading"><span>支払情報</span><Button ref={toggle} type="button" size="icon" variant="ghost" disabled={disabled||showFields&&!canCollapse} title={showFields?'支払情報を折りたたむ':'支払情報を変更'} aria-label={showFields?'支払情報を折りたたむ':'支払情報を変更'} aria-expanded={showFields} aria-controls={contentId} onClick={()=>{setExpanded(!showFields);toggle.current?.focus();}}>{showFields?<ChevronUp />:<Pencil />}</Button></div>}
    {!showFields&&<><dl className="compact-payment-summary"><div><dt>支払元</dt><dd>{source?.name}</dd></div><div><dt>購入・支払いをした人</dt><dd>{fields.usedByText||personLabel(fields.usedByPartyId,masters,true)}</dd></div><div><dt>誰のため</dt><dd>{beneficiaryLabel(fields,masters)}</dd></div></dl>{outcome}</>}
    <div id={contentId} hidden={!showFields}>{showFields&&<div className="payment-controls">
