...
export function LoginForm({ ready, invitation = false }: { ready: boolean; invitation?: boolean }) {
  ...
  async function submit(event?: FormEvent, resend = false) {
    ...
    try {
      if (!invitation && (!sent || resend)) {
        await requestJson('/api/auth/request', { method: 'POST', body: JSON.stringify({ email }) });
        setSent(true); setCooldown(60);
      } else {
        await requestJson('/api/auth/verify', { method: 'POST', body: JSON.stringify({ email, token, type: invitation ? 'invite' : 'email' }) });
  ...
  return <main className="expense-app login-page">
    <div className="login-content">
      ...
      <form onSubmit={submit} className="expense-form">
        ...
        {(sent || invitation) && <>
          {!invitation && <p className="muted-text" role="status">利用可能なアドレスに確認コードを送信しました。</p>}
          <label htmlFor="token">{invitation ? '招待コード' : '確認コード'}</label>
          <input id="token" type="text" inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6,10}" minLength={6} maxLength={10} required autoFocus={!invitation} disabled={!ready || busy} value={token} onChange={e => setToken(e.target.value.replace(/\D/g, ''))} />
        </>}
        {error && <p role="alert" className="form-error">{error}</p>}
        {!ready && <p className="muted-text" role="status">利用アカウントを準備中です。</p>}
        <Button type="submit" disabled={!ready || busy} className="primary-action login-submit">{busy ? <LoaderCircle className="animate-spin" /> : (sent || invitation) ? <ArrowRight /> : <Mail />}{invitation ? '登録してログイン' : sent ? 'ログイン' : '確認コードを送信'}</Button>
      </form>
