...
const profileLabels: Record<string,string> = { nickname:'表示名', profile_confirmed:'本人確認', owner_label:'支払元の識別名' };
...
export function HistoryView({masters,entityType,entityId,revision=0}:{masters:Masters;entityType?:string;entityId?:string;revision?:number}){
  ...
  return <div className="history-view">{!data.events.length?<p className="workspace-message">変更履歴なし</p>:<ul>{data.events.map(event=>{
    const before=event.beforeData??{};const after=event.afterData??{};const labels={...fields,...profileLabels};const keys=Object.keys(labels).filter(k=>(k in before||k in after)&&before[k]!==after[k]);
    const name=after.description||after.name||before.description||before.name||'';
    return <li key={event.id}><details><summary><strong>{entities[event.entityType]} {event.action==='INSERT'?'登録':event.action==='DELETE'?'削除':'変更'} {String(name)}</strong><span>{new Date(event.createdAt).toLocaleString('ja-JP',{timeZone:'Asia/Tokyo'})} · {personLabel(event.actorPartyId,masters)}</span></summary><dl>{keys.map(key=><div key={key}><dt>{labels[key]}</dt><dd>{event.action!=='INSERT'&&<del>{value(key,before[key])}</del>}{event.action==='UPDATE'&&<span aria-hidden="true"> → </span>}{event.action!=='DELETE'&&<span>{value(key,after[key])}</span>}</dd></div>)}</dl></details></li>;
