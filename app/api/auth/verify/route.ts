...
export async function POST(request: Request) {
  try {
    ...
    const input = z.object({ email: z.string().trim().email().max(254), token: z.string().regex(/^\d{6,10}$/) }).strict().parse(await readJson(request));
    if (!allowedEmail(input.email)) throw new ApiError(401, 'コードを確認してください。');
    const client = await authClient();
    const { data, error } = await client.auth.verifyOtp({ ...input, type: 'email' });
