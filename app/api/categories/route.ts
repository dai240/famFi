...
export async function GET() {
  try {
    ...
    return json(await withUserDb(user.id, async tx => (await readMasters(tx)).categories));
...
export async function POST(request: Request) {
  try {
    ...
    const result = await withLedgerDb(user.id, async tx => {
      const existing = await tx.category.findUnique({ where: { userId_id: { userId: user.id, id } } });
      if (existing) {
      ...
      await tx.$executeRaw`insert into famfi.category_entries(user_id,id,name,color,sort_order,parent_id,archived) values (${user.id}::uuid,${id},${input.name},${input.color},${input.sortOrder},${input.parentId},${input.archived})`;
      return tx.category.findUniqueOrThrow({ where: { userId_id: { userId: user.id, id } } });
    });
