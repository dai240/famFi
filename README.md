...
- 本番確認済み: 本人のメールコードログイン、支出の保存・再表示・編集・削除、月別集計・絞り込み、CSV出力、支出を含む暗号化バックアップとローカル復元。初回テストの削除後に登録された実支出1件を保持し、2026-09-10に家計・定期支出の新版を公開した。
- 残作業: 妻のメール確認・招待・実ログイン、実機スマホ・別端末での確認、共通口座の入金/残高、バックアップの別保存先・自動実行。最新状況は [夫婦の家計・定期支出](docs/household-workflow.md)。
