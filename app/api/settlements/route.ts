...
import { sampleBatch, isSampleRecord } from '@/lib/sample-data';
export const dynamic = 'force-dynamic';
export async function GET(request: Request) {
  try {
    const user = await requireUser();
    const query = z.object({ page: z.coerce.number().int().min(1).max(100000).default(1), view: z.enum(['open','all']).default('open'), samples:z.enum(['hide','show']).default('hide') }).strict().parse(Object.fromEntries(new URL(request.url).searchParams));
    const result = await withUserDb(user.id, async (tx, scope) => {
      const isSample=Prisma.sql`(position(${sampleBatch} in e.memo)>0 or starts_with(e.description,'【サンプル】'))`;
      const all = Prisma.sql`from ${dbSchema}.expenses e where e.user_id=${scope.ledgerId}::uuid and e.reimbursement_status='required'`;
      const base = Prisma.sql`${all} ${query.samples==='hide'?Prisma.sql`and not ${isSample}`:Prisma.empty}`;
      ...
      const groups = await tx.$queryRaw<{ fromPartyId: string; toPartyId: string; amount: bigint }[]>(Prisma.sql`select e.reimbursement_from_party_id as "fromPartyId",e.reimbursement_to_party_id as "toPartyId",sum(${balance})::bigint as amount ${all} and not ${isSample} and ${balance}>0 group by e.reimbursement_from_party_id,e.reimbursement_to_party_id`);
      const samples=await tx.$queryRaw<{count:number;amount:bigint}[]>(Prisma.sql`select count(*)::int as count,coalesce(sum(${balance}),0)::bigint as amount ${all} and ${isSample} and ${balance}>0`);
      const rows = await tx.expense.findMany({ where: { id: { in: ids.map(i => i.id) } }, include: expenseInclude, orderBy: [{ date: 'desc' }, { id: 'asc' }] });
      return { ...(await readMasters(tx)), expenses: rows.map(serializeExpense), count: counts[0].count, groups: groups.map(g => ({ ...g, amount: Number(g.amount) })), sampleCount:samples[0].count,sampleAmount:Number(samples[0].amount), unknownCount: await tx.expense.count({ where: { reimbursementStatus: 'unknown' } }) };
    });
    return json(result);
