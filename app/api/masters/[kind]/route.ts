  ...
  try {
    ...
    const result = await withLedgerDb(user.id, async tx => {
      if (kind === 'parties') {
        ...
        await tx.$executeRaw`insert into famfi.parties(id,user_id,name,kind,archived) values (${id}::uuid,${user.id}::uuid,${input.name},${input.kind},${input.archived})`;
      ...
      await tx.$executeRaw`insert into famfi.payment_sources(id,user_id,name,method,funding_party_id,archived) values (${id}::uuid,${user.id}::uuid,${input.name},${input.method},${input.fundingPartyId}::uuid,${input.archived})`;
